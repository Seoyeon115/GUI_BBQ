...
public class ManagerUI implements ActionListener {
	...
	public void init() {
		...
		ImageIcon icon = new ImageIcon("images/bi_bbq.jpg");
		JLabel logo = new JLabel(icon);
		
		JPanel firstmain = new JPanel(new BorderLayout());
		firstmain.setBackground(new Color(255, 255, 255));
		firstmain.add(BorderLayout.CENTER,logo);
		...
		if (obj == btn_modify) {
			...
			if (nameList.size() != 0) {
				for (int j = 0; j < nameList.size(); j++) {
					...
					roomname.setBackground(new Color(255, 255, 255));
